...
namespace TelemetryViewer.GeneratePDS.f120.Session
{
    ...
    {
        public override void ExportToPds(PacketSessionData sessionData, string filePath)
        {
            using (StreamWriter writer = new StreamWriter(filePath))
            {
                // Escrever a seção de dados da sessão
                writer.WriteLine("# Session Data");
                writer.WriteLine("# Columns: Weather, Track Temperature, Air Temperature, Total Laps, Track Length, Session Type, Track ID, Formula, Time Left, Session Duration, Pit Speed Limit, Game Paused, Is Spectating, Spectator Car Index, SLI Pro Native Support, Number of Marshal Zones, Safety Car Status, Network Game, Number of Weather Forecast Samples");
