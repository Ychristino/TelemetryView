    ...
    public class PDSLapData : PDSExport<PacketLapData>
    {
        public override void ExportToPds(PacketLapData packetData, string filePath)
        {
            using (StreamWriter writer = new StreamWriter(filePath))
            {
                ...
                writer.WriteLine("# Columns: Last Lap Time, Current Lap Time, Sector 1 Time, Sector 2 Time, Lap Distance, Total Distance, Safety Car Delta, Car Position, Current Lap Num, Pit Status, Pit Stops, Sector, Current Lap Invalid, Penalties, Warnings, Unserved Drive Through Pens, Unserved Stop Go Pens, Grid Position, Driver Status, Result Status, Pit Lane Timer Active, Pit Lane Time in Lane, Pit Stop Timer, Pit Stop Should Serve Pen");
