...
using System.IO;
using TelemetryViewer.Dicts.f123.Driver;
...
{
    public class PDSParticipants : PDSExport<PacketParticipants>
    {
                ...
                for (int i = 0; i < packetData.m_participants.Length; i++)
                {
                    ...
                    if (i < packetData.m_numActiveCars)
                    {
                        ...
                        writer.WriteLine(string.Format(CultureInfo.InvariantCulture,
                            ...
                            Driver.GetDriverName(participant.m_driverId),               // ID do piloto
                            participant.m_networkId,                                    // ID da rede do jogador
                            Team.GetTeamName(participant.m_teamId),                     // ID do time
                            participant.m_myTeam,                                       // Se é o meu time
                            Nationality.GetNationalityName(participant.m_nationality),  // Nacionalidade do piloto
