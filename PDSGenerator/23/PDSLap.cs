    ...
    public class PDSLap : PDSExport<PacketLap>
    {
        public override void ExportPdsHeader(string filePath)
        {
            using (StreamWriter writer = new StreamWriter(filePath, append: true))
            {
                ...
                writer.WriteLine("# Columns: Car Position, Current Lap, Sector, Last Lap Time (ms), Current Lap Time (ms), " +
                                 "Sector 1 Time (ms), Sector 1 Time (min), Sector 2 Time (ms), Sector 2 Time (min), Delta to Car in Front (ms), " +
                                 "Delta to Race Leader (ms), Lap Distance (m), Total Distance (m), Safety Car Delta (s), Pit Status, Num Pit Stops, " +
                                 "Current Lap Invalid, Penalties (s), Total Warnings, Corner Cutting Warnings, Unserved Drive Through Penalties, " +
                                 "Unserved Stop Go Penalties, Grid Position, Driver Status, Result Status, Pit Lane Timer Active, Pit Lane Time (ms), " +
                                 "Pit Stop Time (ms), Pit Stop Should Serve Penalty");
            }
        }
