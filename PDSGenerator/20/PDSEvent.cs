...
using System.IO;
using TelemetryViewer.Dicts.f120.Infringements;
using TelemetryViewer.Dicts.f120.Penalties;
using TelemetryViewer.Packets.f120.Event;

namespace TelemetryViewer.GeneratePDS.f120.Event
{
    public class PDSEvent : PDSExport<PacketEvent> // Herda de PDSExport<PacketEventData>
    {
        ...
        public override void ExportToPds(PacketEvent eventData, string filePath)
        {
            using (StreamWriter writer = new StreamWriter(filePath, append: true))
            {
                ...
                switch (eventData.m_eventDetails.eventType)
                {
                    ...
                    case EventType.Penalty:
                        writer.WriteLine(string.Format(CultureInfo.InvariantCulture,
                            ...
                            "Penalty",
                            eventCode,
                            Penalty.GetPenaltyMeaning(eventData.m_eventDetails.penalty.penaltyType),
                            Infringement.GetInfringementMeaning(eventData.m_eventDetails.penalty.infringementType),
