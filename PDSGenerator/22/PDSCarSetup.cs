...
namespace TelemetryViewer.GeneratePDS.f122.CarSetup
{
    public class PDSCarSetup : PDSExport<PacketCarSetup>
    {
        public override void ExportPdsHeader(string filePath){
            using (StreamWriter writer = new StreamWriter(filePath, append: true))
            {
                ...
                writer.WriteLine("\n# Car Setup Data (F122)");
                writer.WriteLine("# Columns: Car Index, Front Wing, Rear Wing, On Throttle, Off Throttle, Front Camber, Rear Camber, Front Toe, " +
                                    "Rear Toe, Front Suspension, Rear Suspension, Front Anti Roll Bar, Rear Anti Roll Bar, Front Suspension Height, " +
                                    "Rear Suspension Height, Brake Pressure, Brake Bias, Tyre Pressure RL, Tyre Pressure RR, " +
                                    "Tyre Pressure FL, Tyre Pressure FR, Ballast, Fuel Load");
            }
        }
        public override void ExportToPds(PacketCarSetup packetData, string filePath)
        {
            using (StreamWriter writer = new StreamWriter(filePath, append: true))
            {
                ...
                // Iterar sobre os dados de configuração de todos os carros e exportar para o CSV
                for (int i = 0; i < packetData.m_carSetups.Length; i++)
                {
                    ...
                    writer.WriteLine(string.Format(CultureInfo.InvariantCulture,
                        "{0}, {1}, {2}, {3}, {4}, {5:F2}, {6:F2}, {7:F2}, {8:F2}, {9}, {10}, {11}, {12}, {13}, {14}, {15}, {16:F2}, {17:F2}, {18:F2}, {19:F2}, {20:F2}, {21:F2}",
                        i, // Índice do carro
            ...
            Console.WriteLine($"PacketCarSetupData F122 Sucesso: {filePath}");
        }
    }
}
